...
class Bassoon:
    def __init__(self, master):
        ...
        #add button
        self.addProtocolButton = Button(self.frame, text = 'Add Stimulus', command = lambda: self.addProtocol())
        ...
        self.estimatedTimeLabel = Label(self.frame, text = 'Estimated Time: 0m 0s')
        ...
        self.editProtocolButton = Button(self.buttonFrame, text = 'Edit', command = lambda:self.editProtocol())
        self.editProtocolButton.grid(row = 1, column = 1)
        self.removeProtocolButton = Button(self.buttonFrame, text = 'Remove', command = lambda:self.removeProtocol())
        self.removeProtocolButton.grid(row = 1, column = 2)
        self.saveExperimentButton = Button(self.buttonFrame, text = 'Save', command = lambda:self.saveExperiment())
        self.saveExperimentButton.grid(row = 1, column = 3)
        self.runExperimentButton = Button(self.buttonFrame, text = 'Run Experiment', command = lambda: self.runExperiment())
    ...
    def loadExperiment(self):
        ...
        for p in newExperiment.protocolList:
            pname = p[0]
            pObj = p[1]
            pnameWithSpaces = ''
            ...
            self.experimentSketch.append((pnameWithSpaces, pObj))
    ...
    def listProtocols(self):
        ...
        self.protocolList = [p[:-3] for p in protocolFiles if p.endswith('.py') and p != 'protocol.py' and p != '__init__.py']
    ...
    def changeProtocolIndex(self, e=0):
        ...
        self.protocolIndexField.insert(END, selectionIndex+1) #python is 0 indexed but Bassoon GUI isn't so add 1
    ...
    def addProtocol(self):
        ...
        #insert a tuple into self.experimentSketch. The first index holds the 
        #name of the protocol as will be displayed in the list box. The second index
        #holds the actual stimulus object that will be run during the experiment.
        ...
        or (index_int >= len(self.experimentSketch) and len(self.experimentSketch) != 1):
            ...
            self.experimentSketch.insert(index_int-1, (pnameWithSpaces, protocolObject ))
    ...
    def editExperiment(self):
        ...
        stimulusFrame = LabelFrame(editFrame, text = 'Stimulus Monitor', bd = 5, relief = RIDGE, pady = 10)
        ...
        stimulusMonitorDropdown = OptionMenu(stimulusFrame, self.stimMonitorSelection, *monitorNames)
        stimulusMonitorDropdown.grid(row = 2, column = 2)
        
        #stimulus full screen
        stimulusFullScreenLabel = Label(stimulusFrame, text = 'Full Screen', padx = 10)
        ...
        stimFullScreenChk = Checkbutton(stimulusFrame, var = self.stimFullScreenSelection)
        ...
        stimScreenNumberDropdown = OptionMenu(stimulusFrame, self.stimScreenSelection, *[0, 1, 2, 3])
        stimScreenNumberDropdown.grid(row = 2, column = 6)

        
        #information monitor selection
        informationFrame = LabelFrame(editFrame, text = 'Information Monitor', bd = 5, relief = RIDGE, pady = 10)
        informationFrame.configure(font = ("Helvetica", 12))
        informationFrame.pack()
        
        #use information window
        informationUseLabel = Label(informationFrame, text = 'Use Information Window?')
        ...
        self.informationUseSelection.set(self.experiment.useInformationMonitor == True)
        informationUseChk = Checkbutton(informationFrame, var = self.informationUseSelection) 
        informationUseChk.grid(row = 0, column = 4, pady = (5, 10))
        
        #informatin monitor name
        informationMonitorLabel = Label(informationFrame, text = 'Monitor', padx = 10)
        informationMonitorLabel.grid(row = 2, column = 1)
        self.informationMonitorSelection = StringVar(root)
        self.informationMonitorSelection.set(self.experiment.informationMonitor)
        informationMonitorDropdown = OptionMenu(informationFrame, self.informationMonitorSelection, *monitorNames)
        informationMonitorDropdown.grid(row = 2, column = 2)
        
        #information full screen
        informationFullScreenLabel = Label(informationFrame, text = 'Full Screen', padx = 10)
        informationFullScreenLabel.grid(row = 2, column = 3)
        self.informationFullScreenSelection = IntVar(root)
        self.informationFullScreenSelection.set(self.experiment.informationFullScreen == True)
        informationFullScreenChk = Checkbutton(informationFrame, var = self.informationFullScreenSelection) 
        informationFullScreenChk.grid(row = 2, column = 4)
        
        #information screen number
        informationScreenLabel = Label(informationFrame, text = 'Screen #', padx = 10)
        ...
        informationScreenNumberDropdown = OptionMenu(informationFrame, self.informationScreenSelection, *[0, 1, 2, 3])
        informationScreenNumberDropdown.grid(row = 2, column = 6)
        
        
        #experiment options
        experimentFrame = LabelFrame(editFrame, text = 'Experiment', bd = 5, relief = RIDGE, pady = 10)
        experimentFrame.configure(font = ("Helvetica", 12))
        experimentFrame.pack()
        
        #user initiates protocols
        userInitLabel = Label(experimentFrame, text = 'Manually Initiate Each Protol', padx = 10)
        ...
        writeTtlLabel = Label(experimentFrame, text = 'Write TTL Pulses', padx = 10)
        ...
        recompileLabel = Label(experimentFrame, text = 'Recompile Experiment When Saving', padx = 10)
        ...
        recompileChk = Checkbutton(experimentFrame, var = self.recompileSelection)
        ...
        buttonFrame.pack()
        applyButton = Button(buttonFrame, text = 'Apply Changes', command = self.applyExperimentChanges)
        applyButton.grid(row = 0, column = 0)
        closeButton = Button(buttonFrame, text = 'Close Window', command = lambda:monitorEditWindow.destroy())
    ...
    def editProtocol(self, e=0):
        ...
        editWindow.title('Edit ' + selectedName + ' at Index ' +  str(selectedIndex+1))
        ...
        updateDict = {'propNamesEditable': propNamesEditable, 'propTypes':propTypes, 'entries': entries}
        
        
        #show the estimated time for this particular protocol at the bottom of the window
        protocolRounded_minutes, protocolRemainingSeconds = secondsToMinutesAndSeconds(selectedProtocol.estimateTime())
        self.protocolEstimatedTimeLabel = Label(editFrame, text = 'Estimated Time: ' + str(protocolRounded_minutes) + 'm ' + str(protocolRemainingSeconds) + 's')
        ...
        closeButton = Button(buttonGrid, text = 'Close Window', command = lambda: editWindow.destroy())
    ...
    def applyPropertyChanges(self, selectedIndex, selectedProtocol, updateDict):
        ...
        for i, val in enumerate(userEntries):
            try:
                ...
                elif type(convertToType) == tuple: #will be a tuble when you have a LIST of something
                        splitList = entryString[1:-1].split(',')#first and last are '[' and ']'
                        #second index in the tuple tells you what it's a list of (e.g. list of strings)
                        if convertToType[1] == str:
                            convertedValue = [el.strip() for el in splitList if el != '']
                        elif convertToType[1] == int:
                            convertedValue = [int(el.strip())for el in splitList]
                        elif convertToType[1] == float:
                            convertedValue = [float(el.strip()) for el in splitList]
        ...
        self.experimentSketch[selectedIndex] = (pnameWithSpaces, selectedProtocol)
        ...
        protocolRounded_minutes, protocolRemainingSeconds = secondsToMinutesAndSeconds(selectedProtocol.estimateTime())
        self.protocolEstimatedTimeLabel.configure(text = 'Estimated Time: ' + str(protocolRounded_minutes) + 'm ' + str(protocolRemainingSeconds) + 's')
    ...
    def updateExperimentSketch(self):
        ...
        rounded_minutes, remainingSeconds = secondsToMinutesAndSeconds(estimatedTime_seconds)
        self.estimatedTimeLabel.configure(text = 'Estimated Time: ' + str(rounded_minutes) + 'm ' + str(remainingSeconds) + 's')
    ...
    def compileExperiment(self):
        ...
        self.experiment.protocolList = [] #clear the protocolList in case it has any previous data
    ...
    def saveExperiment(self):
        ...
        expfname = tkfd.asksaveasfilename(defaultextension='.experiment', \
                                       filetypes=[("Experiment Files", '*.experiment')],\
        ...
        try: #LOOK INTO WHY THESE COMMANDS THROW AN ERROR SOMETIMES... MIGHT HAVE TO DO WITH WHEN AN EXPERIMENT IS RELOADED AFTER BEING RUN ONCE
            jsonDict['experimentStartTime'] = jsonDict['experimentStartTime'].strftime("%D %H:%M:%S")
            jsonDict['experimentEndTime'] = jsonDict['experimentEndTime'].strftime("%D %H:%M:%S")
        except:
            pass
        ...
        print('--> .experiment and .json files saved at ' + expfname[0:-11] + '.*')
        try:
    ...
    def runExperiment(self):
        ...
        try:
            ...
            self.saveExperiment() #pass recompile = False becasue you want to save the experiment that you just ran, not a new one
